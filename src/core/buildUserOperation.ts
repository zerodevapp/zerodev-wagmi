...
import { prepareBuildUserOperation } from "./prepareBuildUserOperation.js";
import { GetAccountParameter } from "viem/_types/types/account.js";
import { GetChain } from "viem/_types/types/chain.js";
import {
...
export type BuildUserOperationArgs = {
  /** Chain ID used to validate if the walletClient is connected to the target chain */
  chainId?: number;
  mode?: "prepared";
  to: string | string[];
} & Omit<BuildUserOperationParameters<Chain, Account>, "chain" | "to">;
...
/**
 * @description Function to build a user operation.
 *
 * It is recommended to pair this with the `prepareBuildUserOperation` function to avoid
 * [UX pitfalls](https://wagmi.sh/react/prepare-hooks#ux-pitfalls-without-prepare-hooks).
 *
 * @example
 * import { prepareBuildUserOperation, buildUserOperation } from '@zerdevapp/wagmi'
 *
...
export async function buildUserOperation({
  ...
  to,
  value,
}: BuildUserOperationArgs): Promise<BuildUserOperationResult> {
  /********************************************************************/
  ...
  let args: BuildUserOperationParameters<Chain, Account>;
  if (mode === "prepared") {
    args = {
      account,
      chain: null,
    ...
    args = await prepareBuildUserOperation({
      account,
      chainId,
      data,
      to,
  ...
  if (isArray) {
    ...
    const isValidData =
      args.data === undefined ||
      (Array.isArray(args.data) && args.data.length === args.to.length);
    const isValidValue =
      args.value === undefined ||
      (Array.isArray(args.value) && args.value.length === args.to.length);
    if (!isValidTo || !isValidData || !isValidValue) {
      throw new Error(
        "If one parameter is an array, then all parameters need to be an array."
      );
    }
  }

  //@ts-expect-error
  const request = await walletClient.buildUserOperation(
    isArray
      ? //@ts-expect-error
        args.to.map((target, index) => ({
          target,
          data:
            args.data && Array.isArray(args.data)
              ...
              : undefined,
          value:
            args.value && Array.isArray(args.value)
              ? args.value[index]
              : undefined,
        }))
      : {
